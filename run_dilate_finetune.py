...
flags.DEFINE_float("num_train_epochs", 18.0, "Total number of training epochs to perform.")
...
def convert_single_example(ex_index, example, type2idx, max_seq_length, tokenizer):
    ...
    if ex_index < 5:
        ...
        tf.logging.info("start_labels: %s" % " ".join([str(x) for x in start_label_ids]))
        tf.logging.info("end_labels: %s" % " ".join([str(x) for x in end_label_ids]))
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids, start_labels, end_labels, num_labels, use_one_hot_embeddings):
    ...
    # 125膨胀卷积
    embedding = tf.layers.conv1d(embedding,filters=dim,kernel_size=3,padding="same",dilation_rate=1)
    embedding -= (1.0 - tf.cast(tf.expand_dims(input_mask, 2), tf.float32)) * 1e10
    embedding = tf.layers.conv1d(embedding, filters=dim, kernel_size=3, padding="same", dilation_rate=2)
    embedding -= (1.0 - tf.cast(tf.expand_dims(input_mask, 2), tf.float32)) * 1e10
    embedding = tf.layers.conv1d(embedding, filters=dim, kernel_size=3, padding="same", dilation_rate=5)
    embedding -= (1.0 - tf.cast(tf.expand_dims(input_mask, 2), tf.float32)) * 1e10
    ...
    avgpool=tf.layers.average_pooling1d(embedding,pool_size=dim,padding="same",strides=1)

    # 一维卷积 relu激活
    output=tf.layers.conv1d(avgpool,filters=128,kernel_size=3,activation=tf.nn.relu,padding="same") # [batch_size, max_seq_length, 128]
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,num_train_steps, num_warmup_steps, use_tpu, use_one_hot_embeddings):
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            output_spec = tf.contrib.tpu.TPUEstimatorSpec(
                mode=mode,
                predictions={"start_points": tf.nn.softmax(start_logits,axis=2),"end_points":tf.nn.softmax(end_logits,axis=2)},
                scaffold_fn=scaffold_fn)
        return output_spec
