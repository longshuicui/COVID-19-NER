...
class BLSTM_CRF(object):
    ...
    def add_blstm_crf_layer(self, crf_only):
        ...
        """
        if self.is_training:
            self.embedded_chars = tf.nn.dropout(self.embedded_chars, keep_prob=1.0-self.dropout_rate)
    ...
    def _witch_cell(self):
        ...
        if self.cell_type == 'lstm':
            cell_tmp = tf.keras.layers.LSTMCell(units=self.hidden_unit)
        elif self.cell_type == 'gru':
            cell_tmp = tf.keras.layers.GRUCell(units=self.hidden_unit)
        # 是否需要进行dropout
        if self.dropout_rate is not None:
            cell_tmp = rnn.DropoutWrapper(cell_tmp, output_keep_prob=1.0-self.dropout_rate)
        return cell_tmp
    ...
    def project_crf_layer(self):
        with tf.variable_scope("project_crf"):
            with tf.variable_scope("logits"):
