...
flags.DEFINE_string("bert_config_file", "uncased_L-8_H-512_A-8/bert_config.json", "The config json file")

flags.DEFINE_string("vocab_file", "uncased_L-8_H-512_A-8/vocab.txt", "The vocabulary file that the BERT model was trained on.")

flags.DEFINE_string("output_dir", "./output_dir","The output directory where the model checkpoints will be written.")

## Other parameters

flags.DEFINE_string("init_checkpoint", "uncased_L-8_H-512_A-8/bert_model.ckpt", "Initial checkpoint (usually from a pre-trained BERT model).")

flags.DEFINE_bool("do_lower_case", True, "Whether to lower case the input text.")

flags.DEFINE_integer("max_seq_length", 384, "The maximum total input sequence length")
...
def read_task_examples(input_file,is_training,tokenizer):
    ...
    for eid,entry in enumerate(input_data):
        ...
        example=InputExample(guid=eid,tokens=all_doc_tokens,label=tag_labels)
...
def file_based_convert_examples_to_features(examples, label_list, max_seq_length, output_file, tokenizer):
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    lengths = tf.reduce_sum(used, axis=1)  # [batch_size] 大小的向量，包含了当前batch中的序列长度

    blstm_crf = BLSTM_CRF(embedded_chars=embedding, hidden_unit=256, cell_type="lstm",
                          num_layers=2,
                          dropout_rate=0.1, initializers=initializers, num_labels=num_labels,
                          seq_length=max_seq_length, labels=labels, lengths=lengths, is_training=is_training)
    loss, logits, trans, pred_ids = blstm_crf.add_blstm_crf_layer(crf_only=True)
...
def main(_):
    ...
    label_list = ["O","B-Drug","I-Drug","B-Disease","I-Disease","B-Gene","I-Gene","B-ChemicalCompound",
                  "I-ChemicalCompound","B-Virus","I-Virus","B-Chemical","I-Chemical","B-Phenotype",
                  "I-Phenotype","B-Organization","I-Organization","[CLS]","[SEP]"]
    ...
    if FLAGS.do_train:
        train_examples = read_task_examples(input_file=FLAGS.train_file,is_training=True,tokenizer=tokenizer)
    ...
    if FLAGS.do_train:
        ...
        if not os.path.exists(train_file):
            file_based_convert_examples_to_features(train_examples, label_list, FLAGS.max_seq_length, train_file, tokenizer)
    ...
    if FLAGS.do_predict:
        predict_examples = read_task_examples(FLAGS.dev_file,False,tokenizer)
        num_actual_predict_examples = len(predict_examples)

        predict_file = os.path.join(FLAGS.output_dir, "predict.tf_record")
        if not os.path.exists(predict_file):
            file_based_convert_examples_to_features(predict_examples, label_list, FLAGS.max_seq_length, predict_file, tokenizer)
        ...
        with tf.gfile.GFile(output_predict_file, "w") as writer:
            ...
            for (i, prediction) in enumerate(result):
                predict_ids=prediction["outputs"]
                doc_tokens=" ".join(predict_examples[i].tokens)
                doc_labels=" ".join([label_list[idx] for idx in predict_ids])
                writer.write(doc_tokens+"\t"+doc_labels+"\n")
