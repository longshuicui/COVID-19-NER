...
flags.DEFINE_string(
    "bert_config_file", "uncased_L-8_H-512_A-8/bert_config.json",
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string("task_name", "domain", "The name of the task to train.")

flags.DEFINE_string("vocab_file", "uncased_L-8_H-512_A-8/vocab.txt",
...
flags.DEFINE_string(
    "init_checkpoint", "uncased_L-8_H-512_A-8/bert_model.ckpt",
...
def file_based_convert_examples_to_features(examples, label_list, max_seq_length, output_file):
...
def main(_):
    ...
    if FLAGS.do_predict:
        predict_examples, predict_data = processor.get_predict_examples(FLAGS.data_dir)
        ...
        with tf.gfile.GFile(output_predict_file, "w") as writer:
            ...
            for (i, prediction) in enumerate(result):
                print(prediction)
                predict_data[i]["type"] = np.argmax(prediction["probabilities"]).astype(np.int32)
                if i >= num_actual_predict_examples:
                    break
                num_written_lines += 1
            json.dump(predict_data, writer, ensure_ascii=False, indent=4)
